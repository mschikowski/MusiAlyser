...
public:
	...
	std::string GetDirectory ()
	{
		...
		dir.resize (GetDirLen ());
		return dir;
	}

    bool GetExistingFile (HWND hwnd, char const * title = 0);

    bool GetNewFile (HWND hwnd, char const * title = 0);

    bool GetMultipleFiles (HWND hwnd, char const * title = 0);
